...
CREATE TABLE `litemall_group` (
  ...
  `type` tinyint(3) NOT NULL DEFAULT '7' COMMENT '类型',
...
CREATE TABLE `litemall_activity` (
  ...
  `latitude` decimal (10,6) DEFAULT NULL,
  `longitude` decimal (10,6) DEFAULT NULL,
  `duration` int(11) DEFAULT '21600' COMMENT '活动时间6小时',
...
CREATE TABLE `litemall_fish_ponds` (
  ...
  `title` varchar(255) NOT NULL,
  `description` varchar(1023) NOT NULL COMMENT '描述',
  `type` tinyint(3) NOT NULL DEFAULT '5' COMMENT '类型',
  `rule` int(11) DEFAULT NULL COMMENT '规则',
  `reword` int(11) DEFAULT NULL COMMENT '奖励',
  `fee` int(11) DEFAULT NULL COMMENT '报名费',
  `limited` int(11) DEFAULT NULL COMMENT '人数限制',
  `collectionCount` int(11) DEFAULT NULL,
  `credit` int(11) DEFAULT NULL,
  `level` int(11) DEFAULT NULL COMMENT '级别',
  `current_people` int(11) DEFAULT NULL,
...
CREATE TABLE `litemall_question` (
  ...
  `title` varchar(255) NOT NULL,
  `description` varchar(1023) NOT NULL COMMENT '描述',
  `type` tinyint(3) NOT NULL DEFAULT '6' COMMENT '类型',
...
CREATE TABLE `litemall_circle` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `user_id` int(11) NOT NULL DEFAULT '0' COMMENT '用户表的用户ID',
  `type` tinyint(3) NOT NULL DEFAULT '4' COMMENT '类型',
...
CREATE TABLE `litemall_ad_rent` (
  ...
  `type` tinyint(3) NOT NULL DEFAULT '0' COMMENT '类型',
  `content` varchar(100) DEFAULT NULL COMMENT '回答内容',
  `pic_urls` varchar(1023) DEFAULT NULL COMMENT '背景图',
  `link_url` varchar(1023) DEFAULT NULL COMMENT '背景图',
  `position` tinyint(3) DEFAULT NULL COMMENT '回答内容',
  `status` smallint(6) DEFAULT '0' COMMENT '状态，如果是0则是正常可用；如果是1则是过期; 如果是2则是下架。',
  `add_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '创建时间',
  `deleted` tinyint(1) DEFAULT '0' COMMENT '逻辑删除',
