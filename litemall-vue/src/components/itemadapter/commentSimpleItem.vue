<template>
    <div class="item" v-if="comment!==undefined">
        ...
        <div class="comment">
            ...
            <div class="imgbox"
                 v-show="comment.comment.picUrls.length>0">
                <img alt=""
                     v-if="comment.comment.picUrls.length== 1"
                     :src="comment.comment.picUrls[0]"
                     :class="{releaseimgone:true}" @click="previewimage(comment.comment.picUrls,0)"/>
                <img alt=""
                     v-else-if="comment.comment.picUrls.length>= 2&&Math.round(comment.comment.picUrls.length%2)==0"
                     v-for="(value,imgIndex) in comment.comment.picUrls"
                     :key="imgIndex"
                     :src="value"
                     :class="{releaseimgtwo:true}" @click="previewimage(comment.comment.picUrls,imgIndex)"/>
                <img alt=""
                     v-else-if="comment.comment.picUrls.length>= 2&&Math.round(comment.comment.picUrls.length%2)!=0"
                     v-for="(value,imgIndex) in comment.comment.picUrls"
                     :key="imgIndex"
                     ...
                     @click="previewimage(comment.comment.picUrls,imgIndex)"/>
            </div>
            <div class="time">{{comment.comment.addTime}}</div>
